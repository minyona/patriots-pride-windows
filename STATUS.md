...
**New logo (2026-07-14):** high-res brand logo swapped in sitewide. `images/logo.webp` (860×371, 50KB) = seal + wordmark with the wordmark recolored flat bone via its own alpha (source wordmark was navy — invisible on the navy header tab / footer / drawer surfaces where the logo renders). New square `images/logo-seal.webp` (512×512, 44KB) = seal only, now used for the favicon and the Organization schema `logo` (both in build.py `doc()`/`org_schema()`). Header/drawer/footer `<img>` width attrs updated to the new 2.32 aspect (121×52 / 121×52 / 172×74). Source master: `~/Downloads/patriots-pride-windows-logo.png` (3082×1376 transparent PNG); the navy-wordmark original is NOT in the repo — regenerate variants from that master if needed.
